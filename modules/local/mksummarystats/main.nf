...
process MKSUMMARYSTATS {
    ...
    container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
        'oras://community.wave.seqera.io/library/matplotlib_numpy_pandas_python_pruned:55ae5840363c9777':
    ...
    tuple val(meta), path("virtus_outputs/*")
    path(samplesheet)

    output:
    // TODO nf-core: Named file extensions MUST be emitted for ALL output channels
    tuple val(meta), path("*.csv"), emit: summary
    tuple val(meta), path("*.pdf"), emit: scattermap
    ...
    tuple val("${task.process}"), val('statsmodels'), eval("python3 -c 'import statsmodels; print(statsmodels.__version__)'"), topic: versions, emit: versions_statsmodels
