...
process BAMFILTERPOLYX {
    ...
    container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
        'oras://community.wave.seqera.io/library/samtools:1.23--86cd9d13645d4fff':
    ...
    output:
    // TODO nf-core: Update the information obtained from bio.tools and make sure that it is correct
    tuple val(meta), path("*.bam"), emit: sequence_trace
    ...
    tuple val("${task.process}"), val('grep'), eval("grep --version | sed '1!d; s/grep (GNU grep) //'"), topic: versions, emit: versions_grep
    ...
    """
    samtools view -h -@ $task.cpus $sequence_trace | \\
        grep -v "AAAAAAAAAAAAAAAAAAAA" | \\
        grep -v "TTTTTTTTTTTTTTTTTTTT" | \\
        grep -v "TGTGTGTGTGTGTGTGTGTGTGTGTGTGTGTG" | \\
